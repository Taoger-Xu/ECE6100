...
void print_stats(void) {
	...
	uint64_t cache_lookups = pipeline->pipeline_cache->lookups;
	uint64_t cache_hits = pipeline->pipeline_cache->hits;
	double hit_ratio = (double)(cache_hits) / (double)(cache_lookups);
	...
	printf("\n%s_Cache HR           \t : %10.3f", header, hit_ratio);
